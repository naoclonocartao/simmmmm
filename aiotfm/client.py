...
from aiotfm.utils import Keys, Locale, get_keys, shakikoo, solve_captcha
...
class Client:
	...
	async def handle_packet(self, connection: Connection, packet: Packet) -> bool:
		...
		if CCC == (1, 1): # Old packets
			...
			if player.moving_right or player.moving_left:
				player.facing_right = player.moving_right
			...
			if xml:
				...
				self._room.map.spawn_pos = get_data(xml, 'DS', reversed=is_reversed)
			else:
				self._room.map.xml = ""
		...
		if CCC == (5, 21): # Joined room
			...
			captcha = await solve_captcha(zlib.decompress(packet.readBytes(packet.read32())))
			if captcha is None:
				print("Received invalid CAPTCHA")
			else:
				self.dispatch('new_captcha', captcha)
			...
			if TC == 3: # Connected to the community platform
			...
			elif TC == 85: # Tribe creation
				packet.read32()
				self.dispatch('tribe_creation', packet.read8())
				...
				self.dispatch('member_joined', packet.readUTF())

			elif TC == 92:
				self.dispatch('member_left', packet.readUTF())

			elif TC == 93:
				self.dispatch('member_kick', packet.readUTF(), packet.readUTF())
			...
			packet = Packet.new(176, 4)
			for key in self.keys.credentials_key:
				packet.write32(n).writeUTF(key)
			packet.write16(22).write16(88)
	...
	async def handle_old_packet(self, connection: Connection, oldCCC: tuple, data: list) -> bool:
		...
		if oldCCC == (8, 5): # Player died
		...
		elif oldCCC == (26, 17): # Ban info
			self._logged = False
			self.loop.call_later(3, self.close)
			self.dispatch('ban_info', data[1], int(data[0]) / 3600000)

		elif oldCCC == (26, 18): # Ban info
			time = 0
			if len(data) >= 2:
				time = int(data[0]) / 3600000
				reason = data[1]
			else:
				reason = data[0]
			...
			self._logged = False
			self.loop.call_later(3, self.close)
			self.dispatch('ban_info', reason, time)
	...
	async def register(
		...
		packet.writeString(email).writeString(captcha).writeString("")
	...
	async def create_tribe(self, name: str):
		await self.sendCP(84, Packet().write32(self.pid).writeUTF(name))
	...
	async def recruit(self, username: Union[Player, str]):
		...
		"""
		await self.sendCP(78, Packet().writeString(str(username)))

	async def kickMember(self, username: Union[Player, str]):
		await self.sendCP(104, Packet().writeString(str(username)))
