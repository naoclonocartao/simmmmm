...
class Command(Message):
	def __init__(self, author: Player, content: str, platform: str, client: 'aiotfm.Client'):
		super().__init__(author, content, client)
		
		self.platform = platform
		
		self.name = content.split(' ', 1)[0][1:]
		
		self.args = content.split()[1:]
		if len(self.args) == 0:
			self.args.append(author)
...
class Whisper(Message):
	...
	community: :class:`aiotfm.enums.Community`
...
class ChannelMessage(Message):
	...
	def __str__(self):
		return '(#{0.channel.name}) [{0.community.name}] [{0.author}] {0.content}'.format(self)